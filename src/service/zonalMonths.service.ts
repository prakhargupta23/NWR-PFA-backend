...
/**
 * Returns a sorted list of unique months (MM/YYYY) present in the ZonalData table.
 * The list is sorted in descending chronological order (most recent first).
 */
export async function getUniqueZonalMonths(): Promise<string[]> {
    // Use a raw query for efficiency – SELECT DISTINCT i   s far cheaper than
    // pulling every row and de-duplicating in JavaScript.
    const rows = await sequelize.query<{ selectedMonthYear: string }>(
        `SELECT DISTINCT selectedMonthYear FROM ZonalData WHERE selectedMonthYear IS NOT NULL`,
